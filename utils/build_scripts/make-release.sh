...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/xtendcash/xtendcash/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin xtend-$build-x64-$version
    zip -r xtend-$build-x64-$version.zip xtend-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum xtend-*-x64-$version.zip
