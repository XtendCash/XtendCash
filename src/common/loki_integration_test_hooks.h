...
#ifndef XTEND_INTEGRATION_TEST_HOOKS_H
#define XTEND_INTEGRATION_TEST_HOOKS_H
...
#endif // XTEND_INTEGRATION_TEST_HOOKS_H

//
// CPP Implementation
//
#ifdef XTEND_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
#include <string.h>
#include <assert.h>
#include <chrono>
#include <thread>
...
, "xtend-default-integration-test-mem-name"
, false
};
...
void xtend::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void xtend::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void xtend::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("xtend Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "Xtend Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "Xtend Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "Xtend Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "Xtend Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("Xtend Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void xtend::deinit_integration_test_context()
{
...
std::vector<std::string> xtend::separate_stdin_to_space_delim_args(xtend::fixed_buffer const *cmd)
{
...
xtend::fixed_buffer xtend::read_from_stdin_shared_mem()
{
...
void xtend::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
...
#endif // XTEND_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
